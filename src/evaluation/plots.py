...
def plot_ic_time_series(ic_dict, ax=None):
    ...
    """
    fig, ax = (plt.subplots(figsize=(12, 4)) if ax is None else (None, ax))
...
def plot_decile_spread(lf, signal_col, fwd_ret_col='fwd_ret', n_buckets=10, ax=None):
    """
    ...
    fig, ax = (plt.subplots(figsize=(6, 4)) if ax is None else (None, ax))
